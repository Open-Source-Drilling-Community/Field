...
class Program
{
    ...
    private static readonly string JSON_BUNDLE = "FieldMergedModel.json";
    private static readonly string CSHARP_MODEL = "FieldMergedModel.cs";
            ...
            if (res == "Y")
            {
                try
                {
                    if (Directory.Exists(jsonInputsDirectory))
                    {
                        ...
                        // same fix applied in Service/SwaggerMiddlewareExtensions.cs
